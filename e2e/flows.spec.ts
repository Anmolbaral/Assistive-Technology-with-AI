...
async function completeQuiz(page: Page, lessonSlug: string): Promise<boolean> {
  ...
  // Wait for the quiz to render inside the MDX content
  const quizHeading = page.locator("text=Knowledge Check");
  ...
  for (let q = 0; q < answers.length; q++) {
    ...
    // Wait for the current question to render — verify question counter
    await expect(
      page.locator(`text=Question ${q + 1} of ${answers.length}`)
...
test.describe("Full Learning Flow — Real Quiz Completion", () => {
  test("user completes all 4 quizzes and unlocks the assistant", async ({ page }) => {
    ...
    await page.locator("text=Start Training").first().click();
    ...
    await expect(
      page.locator("h1", { hasText: "TechBridge Learning AT Resource Assistant" })
    ).toBeVisible();
  });
});
...
test.describe("Quiz Failure and Retry Flow", () => {
  test("failing a quiz shows retry button and user can retake", async ({ page }) => {
    ...
    // Scroll to quiz
    await page.locator("text=Knowledge Check").scrollIntoViewIfNeeded();
    await expect(page.locator("text=Knowledge Check")).toBeVisible({ timeout: 10000 });
    ...
    for (let q = 0; q < wrongAnswers.length; q++) {
      await expect(
        page.locator(`text=Question ${q + 1} of ${wrongAnswers.length}`)
      ).toBeVisible({ timeout: 5000 });
...
test.describe("Role Selection Flow — Real Interaction", () => {
  test("role selection dialog appears and choosing a role updates the experience", async ({ page }) => {
    ...
    const specialistBtn = page.locator('button[aria-pressed="true"]');
    await expect(specialistBtn).toContainText("AT Specialist");
  ...
  test("role persists across full page navigation", async ({ page }) => {
    ...
    // Verify AT Specialist is active
    const specialistBtn = page.locator('button[aria-pressed="true"]');
    await expect(specialistBtn).toContainText("AT Specialist");
    ...
    // Role should still be AT Specialist
